...
export const DealsSection = () => (
  <div>
    <div className="flex max-w-5xl flex-col gap-7 lg:flex-row lg:items-end lg:justify-between">
      <div>
        <p className="text-sm font-extrabold uppercase tracking-[0.12em] text-(--red)">Deals & featured picks</p>
    ...
    <div className="mt-12 grid gap-5 lg:grid-cols-3">
      {featureSlots.map((slot) => (
        <article key={slot.title} className="relative flex min-h-80 flex-col overflow-hidden rounded-[7px] border border-[#dfbfc1] bg-white p-7 sm:p-8">
          <span className="absolute inset-x-0 top-0 flex h-1" aria-hidden="true"><span className="w-4/5 bg-(--red)" /><span className="flex-1 bg-(--accent)" /></span>
          <div className="flex items-center justify-between gap-4">
            <p className="text-xs font-extrabold uppercase tracking-[0.12em] text-(--red)">{slot.eyebrow}</p>
            <span className="h-3 w-10 bg-(--accent)" aria-hidden="true" />
          </div>
  ...
  <div className="grid gap-12 lg:grid-cols-[1.22fr_0.78fr] lg:items-center">
    <div className="relative min-h-[420px] overflow-hidden rounded-[7px] border border-[#3a3a3f] bg-[#18181b] sm:min-h-[520px] lg:min-h-[590px]">
      ...
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#0b0b0d] via-[#0b0b0d]/70 to-transparent p-7 pt-36 sm:p-9 sm:pt-40">
        <p className="text-sm font-extrabold uppercase tracking-[0.1em] text-[#ff5a65]">A real local store on IN-39</p>
    ...
    <div>
      <p className="text-sm font-extrabold uppercase tracking-[0.1em] text-[#ff5a65]">About Big Wicks</p>
      ...
      <p className="mt-7 text-lg leading-8 text-[#d0d0cd]">Big Wicks is one of Indiana&apos;s newer retail fireworks destinations, located in La Porte just 3 miles south of downtown New Buffalo. Inside, you&apos;ll find an organized store, a broad selection, and people ready to help.</p>
      <p className="mt-5 leading-7 text-[#aaa9a5]">Whether you&apos;re planning a family celebration or looking for something with more impact, the goal is simple: make it easier to leave with fireworks you&apos;re excited to light.</p>
  ...
  <div>
    <div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
      <div className="max-w-4xl">
        <p className="text-sm font-extrabold uppercase tracking-[0.1em] text-(--red)">Proof of selection</p>
        <h2 className="text-balance mt-4 font-heading text-5xl font-bold uppercase leading-[0.9] text-[#171719] md:text-7xl">That place is loaded</h2>
      </div>
      <p className="max-w-md leading-7 text-[#625f5b]">Bright shelves, long aisles, and options for different kinds of celebrations. The variety is easier to understand when you see the real store.</p>
    </div>
    ...
    <div className="mt-12 grid gap-5 lg:grid-cols-[1.35fr_0.65fr]">
      ...
      <figure className="relative min-h-[480px] overflow-hidden rounded-[7px] border border-[#d5d5d0] lg:min-h-[640px]">
        ...
        <div className="absolute inset-x-0 top-0 flex h-1.5" aria-hidden="true"><span className="w-4/5 bg-(--red)" /><span className="flex-1 bg-(--accent)" /></div>
...
const ValueIcon = ({ index }: { index: number }) => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="size-7" aria-hidden="true">
    {index === 0 ? (
      ...
      <><path d="M20 13 11 22l-9-9V3h10Z" /><circle cx="7" cy="8" r="1.5" /></>
    )}
  </svg>
);
...
export const WhyChooseSection = () => (
  <div>
    <div className="grid gap-8 lg:grid-cols-[1fr_0.65fr] lg:items-end">
      <div>
        <p className="text-sm font-extrabold uppercase tracking-[0.1em] text-(--red)">Customer feedback themes</p>
    ...
    <div className="mt-12 grid gap-5 lg:grid-cols-3">
      {customerPriorities.map((item, index) => (
        <article key={item.title} className="relative min-h-72 overflow-hidden rounded-[7px] border border-[#d5d5d0] bg-white p-7 sm:p-8">
          <span className="absolute inset-x-0 top-0 h-1 bg-(--red)" aria-hidden="true" />
          <div className="flex size-14 items-center justify-center rounded-[5px] bg-[#fff0f1] text-(--red)">
            <ValueIcon index={index} />
          </div>
          <h3 className="mt-7 font-heading text-2xl font-bold uppercase leading-none text-[#171411]">{item.title}</h3>
...
export const DemoSection = () => (
  <div className="grid gap-12 lg:grid-cols-[1.08fr_0.92fr] lg:items-center">
    ...
    <div>
      <p className="text-sm font-extrabold uppercase tracking-[0.1em] text-[#ff5a65]">See the effect</p>
      ...
      <p className="mt-7 text-lg leading-8 text-[#d0d0cd]">A package can only tell you so much. Big Wicks has used the large in-store TV to help customers see how particular fireworks look when fired.</p>
      <p className="mt-5 leading-7 text-[#aaa9a5]">Ask our team whether a demonstration is available for the item you&apos;re considering. This does not imply that every product currently has a video.</p>
      <div className="mt-7 rounded-[7px] border border-[#3a3a3f] bg-[#202024] p-5">
        ...
        <p className="mt-2 text-sm leading-6 text-[#c2c2bf]">The product-page architecture can add verified demo videos later without changing this in-store promise.</p>
      </div>
