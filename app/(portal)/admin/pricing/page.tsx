  ...
  return <>
    ...
    {result.status === "unavailable" ? <div className="mt-6 border-l-4 border-(--red) bg-white p-5"><p role="status">{result.message}</p><a href="/admin/pricing" className="mt-4 inline-flex min-h-11 items-center font-semibold underline">Try again</a></div> : <>
      <dl className="mt-6 grid grid-cols-2 gap-4 border-y border-(--border) py-5 sm:grid-cols-3">
        {[["Sanity products", result.counts.total], ["Available products", result.counts.available], ["Fully priced products", result.counts.fullyPriced], ["Missing Tier 1", result.counts.missingTier1], ["Missing Tier 2", result.counts.missingTier2], ["Audit problems", result.issues.length]].map(([label, value]) => <div key={label}><dt className="text-sm text-(--muted)">{label}</dt><dd className="mt-1 font-heading text-3xl font-bold">{value}</dd></div>)}
      </dl>
      <p className="mt-3 text-sm text-(--muted)">Completeness counts include visible and hidden products with valid identities. Availability is manual catalog visibility.</p>
      ...
      <section aria-labelledby="current-pricing" className="mt-10">
        <h2 id="current-pricing" className="font-heading text-2xl font-bold">Current product prices</h2>
        ...
        {result.rows.length === 0 ? <p className="mt-5">No valid published products are available to price.</p> : <div className="mt-5 overflow-x-auto border border-(--border) bg-white" role="region" aria-label="Current pricing table" tabIndex={0}>
          <table className="w-full min-w-[760px] text-left text-sm"><caption className="sr-only">Current Sanity products and private tier prices</caption>
            <thead><tr>{["SKU / Product", "Category", "Visibility", "Tier 1", "Tier 2", "Status"].map((label) => <th key={label} scope="col" className="border-b border-(--border) p-3">{label}</th>)}</tr></thead>
            <tbody>{result.rows.map((row) => <tr key={row.catalogKey} className="border-b border-(--border) last:border-0"><th scope="row" className="max-w-64 p-3 font-normal wrap-anywhere"><strong>{row.sku}</strong><div>{row.name}</div></th><td className="max-w-40 p-3 wrap-anywhere">{row.category || "Uncategorized"}</td><td className="p-3">{row.available ? "Visible" : "Hidden"}</td><td className="p-3 tabular-nums whitespace-nowrap">{row.tier1Price ?? "No price"}</td><td className="p-3 tabular-nums whitespace-nowrap">{row.tier2Price ?? "No price"}</td><td className="p-3">{row.tier1Price !== null && row.tier2Price !== null ? "Complete" : "Missing price"}</td></tr>)}</tbody>
          </table>
        </div>}
      </section>
    </>}
