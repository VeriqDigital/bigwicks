...
import { getCustomer, listCustomerTiers, hasPendingSetup } from "@/lib/admin/customers";
...
export default async function EditCustomerPage({ params }: { params: Promise<{ customerId: string }> }) {
  ...
  const pendingSetup = !customer.passwordSet && await hasPendingSetup(customerId);
  return <>
    ...
    <section className="mt-12 max-w-xl border-t border-[var(--border)] pt-6" aria-labelledby="access-heading">
      ...
      {!customer.passwordSet && <><p className="mt-2">Password not set. This account cannot sign in yet.</p><InviteForm key={`${customer.email}:${customer.active}`} customerId={customer.id} pendingSetup={pendingSetup} /></>}
