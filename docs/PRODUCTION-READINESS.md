...
| `ALLOW_DEVELOPMENT_SEED` | Operator / no | C / — / — | Exact `true`, `prisma/seed.ts`; only local fixtures. | Default false/absent refuses. Production NODE_ENV refuses; the local target is validated before hashing/connecting and passed as explicit driver fields, with routing overrides/socket URLs rejected. See [local seed format](AUTH.md#local-setup). A local tunnel can defeat target intent; **never seed production through a tunnel**. |
