...
import { mkdirSync, mkdtempSync, copyFileSync, symlinkSync, readdirSync } from 'node:fs';
...
if (!command) throw new Error('Choose lint, typecheck, integration, unit, or focused with an existing isolated build directory.');
...
if (process.argv[2] === 'focused') {
  const built = resolve(sourceRoot, process.argv[3] ?? '');
  if (!built.startsWith(resolve(sourceRoot, '.test-runtime') + '\\')) throw new Error('Use an isolated audit build.');
...
const steps = process.argv[2] === 'typecheck'
  ...
  : [[...command, ...(process.argv[2] === 'focused' ? [] : process.argv.slice(3))]];
